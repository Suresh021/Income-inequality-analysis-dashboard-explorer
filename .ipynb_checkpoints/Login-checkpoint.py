...
def show():
    ...
    # --- CSS for buttons (works for both st.button and st.form_submit_button) ---
    st.markdown("""
    <style>
    div.stButton > button,
...
with col2:
    ...
    # 🔹 App Title inside a container (card style like your dashboard)
    st.markdown("""
    <div style="
        ...
        max-width: 1000px;    /* keeps it long but not too stretched */
        margin-left: auto;
        margin-right: auto;
    ">
    ...
    # 🔹 Subheader (kept from your code)
    st.markdown("<p style='text-align:center; color:gray;'>Welcome back! Please login to continue.</p>", unsafe_allow_html=True)
        ...
        if login_submitted:
            if not username or not password:
                ...
                with st.spinner("🔵 Checking credentials..."):
                    ...
                    # 1️⃣ Check if username exists in Firestore
                    ...
                    if not user_doc.exists:
                        ...
                        if not email:
                            ...
                            # 2️⃣ Authenticate with Firebase using email + password
                            try:
                                result = verify_user_with_password(email, password)
                                if "idToken" in result:  # ✅ Login success
                                    ...
                                    error_message = result.get("error", {}).get("message", "❌ Wrong credentials")
